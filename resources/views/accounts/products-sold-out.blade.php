...
<div class="max-w-7xl mx-auto px-4 py-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-semibold text-gray-800">Productos sin cupo disponible</h1>
            <p class="text-sm text-gray-500">
                Productos PS4/PS5 que tienen cuenta(s) pero sin cupos libres para vender.
            </p>
        </div>

        <a href="{{ route('accounts.products-sold-out.export', request()->query()) }}"
    ...
    <form method="GET" class="mb-4 flex flex-wrap items-end gap-3">
        ...
        @if (request()->hasAny(['search', 'platform']))
            <a href="{{ route('accounts.products-sold-out') }}"
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($products as $product)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">
                            @if ($product->game)
                                <a href="{{ route('accounts.index', ['game_id' => $product->game_id]) }}"
                                   target="_blank"
                        ...
                        <td class="px-4 py-3 text-gray-600">
                            <span class="font-medium text-gray-800">{{ $product->accounts_active }}</span> activa(s)
                            @if ($product->accounts_blocked)
                                <span class="ml-1 text-xs text-amber-600">+{{ $product->accounts_blocked }} bloqueada(s)</span>
                            @endif
                        </td>
                        {{-- <td class="px-4 py-3">
                            <span class="inline-flex rounded-full bg-rose-50 px-2.5 py-0.5 text-xs font-medium text-rose-700">
                                {{ $product->used_total }}/{{ $product->capacity_total }} · 0 libres
                            </span>
                        </td> --}}
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-10 text-center text-gray-400">
                            No hay productos sin cupo con estos filtros. 🎉
                        </td>
                    </tr>
                @endforelse
            </tbody>
