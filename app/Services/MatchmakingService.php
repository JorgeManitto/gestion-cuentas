...
class MatchmakingService
{
     ...
     */
    public function assignSecondaryMany(OrderItem $item, array $accountIds, string $whoDelivered): array
    {
        ...
        foreach ($accounts as $account) {
            $sa = $this->assignSecondarySlot($item, $account);
            if ($sa) {
                $assigned[] = $sa;
            } else {
    ...
    private function assignSecondarySlot(OrderItem $item, Account $account): ?AccountSecondaryAssignment
    {
        ...
        return DB::transaction(function () use ($item, $account, $freeSlot, $slotPlatform, $key) {
